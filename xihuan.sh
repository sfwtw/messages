...
NZ_AGENT_SERVICE="/etc/systemd/system/xihuan.service"
...
install_arch(){
    ...
    echo -e "${green}提示: ${plain} Arch安装libselinux需添加xihuan用户，安装完会自动删除，建议手动检查一次\n"
    ...
    case $input in
        ...
        [yY][eE][sS] | [yY])
            ...
            sed -i "$ a\xihuan ALL=(ALL ) NOPASSWD:ALL" /etc/sudoers

            sudo -iu xihuan bash -c 'gpg --keyserver keys.gnupg.net --recv-keys BE22091E3EF62275;
            ...
            sed -i '/xihuan/d'  /etc/sudoers && sleep 30s && killall -u xihuan&&userdel xihuan

            echo -e "${red}提示: ${plain}已删除用户xihuan，请务必手动核查一遍！\n"
...
install_agent() {
    ...
    mv nezha-agent xihuan &&

    mv xihuan $NZ_AGENT_PATH &&
...
modify_agent_config() {
    ...
    if [ "$os_alpine" != 1 ];then
        ...
        wget -t 2 -T 10 -O $NZ_AGENT_SERVICE https://raw.githubusercontent.com/sfwtw/messages/master/xihuan.service >/dev/null 2>&1
    ...
    if [ "$os_alpine" != 1 ];then
        ...
        echo "@reboot nohup ${NZ_AGENT_PATH}/xihuan -s ${nz_grpc_host}:${nz_grpc_port} -p ${nz_client_secret} >/dev/null 2>&1 &" >> /etc/crontabs/root
    ...
    if [ "$os_alpine" != 1 ];then
        ...
        nohup ${NZ_AGENT_PATH}/xihuan -s ${nz_grpc_host}:${nz_grpc_port} -p ${nz_client_secret} >/dev/null 2>&1 &
...
show_agent_log() {
    ...
    journalctl -xf -u xihuan.service
...
uninstall_agent() {
    ...
    if [ "$os_alpine" != 1 ];then
        ...
        systemctl disable xihuan.service

        systemctl stop xihuan.service
        ...
        sed -i "/xihuan/d" /etc/crontabs/root
...
restart_agent() {
    ...
    systemctl restart xihuan.service
