...
NZ_AGENT_SERVICE="/etc/systemd/system/nezha-agent-cdn.service"
...
install_arch(){
    ...
    echo -e "${green}提示: ${plain} Arch安装libselinux需添加nezha-agent-cdn用户，安装完会自动删除，建议手动检查一次\n"
    ...
    case $input in
        ...
        [yY][eE][sS] | [yY])
            ...
            useradd -m nezha-agent-cdn

            sed -i "$ a\nezha-agent666 ALL=(ALL ) NOPASSWD:ALL" /etc/sudoers

            sudo -iu nezha-agent-cdn bash -c 'gpg --keyserver keys.gnupg.net --recv-keys BE22091E3EF62275;
            ...
            sed -i '/nezha-agent-cdn/d'  /etc/sudoers && sleep 30s && killall -u nezha-agent-cdn&&userdel nezha-agent-cdn

            echo -e "${red}提示: ${plain}已删除用户nezha-agent-cdn，请务必手动核查一遍！\n"
...
install_agent() {
    ...
    mv nezha-agent nezha-agent-cdn &&

    mv nezha-agent-cdn $NZ_AGENT_PATH &&
...
modify_agent_config() {
    ...
    if [ "$os_alpine" != 1 ];then
        ...
        wget -t 2 -T 10 -O $NZ_AGENT_SERVICE https://raw.githubusercontent.com/sfwtw/messages/master/nezha-agent-cdn.service >/dev/null 2>&1
    ...
    if [ "$os_alpine" != 1 ];then
        ...
        echo "@reboot nohup ${NZ_AGENT_PATH}/nezha-agent-cdn -s ${nz_grpc_host}:${nz_grpc_port} -p ${nz_client_secret} >/dev/null 2>&1 &" >> /etc/crontabs/root
    ...
    if [ "$os_alpine" != 1 ];then
        ...
        systemctl enable nezha-agent-cdn

        systemctl restart nezha-agent-cdn

    else

        nohup ${NZ_AGENT_PATH}/nezha-agent-cdn -s ${nz_grpc_host}:${nz_grpc_port} -p ${nz_client_secret} >/dev/null 2>&1 &
...
show_agent_log() {
    ...
    journalctl -xf -u nezha-agent-cdn.service
...
uninstall_agent() {
    ...
    if [ "$os_alpine" != 1 ];then
        ...
        systemctl disable nezha-agent-cdn.service

        systemctl stop nezha-agent-cdn.service
        ...
        sed -i "/nezha-agent-cdn/d" /etc/crontabs/root
...
restart_agent() {
    ...
    systemctl restart nezha-agent-cdn.service
